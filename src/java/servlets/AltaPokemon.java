...
public class AltaPokemon extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Integer atk = Integer.parseInt(request.getParameter("ataque"));
            Integer def = Integer.parseInt(request.getParameter("defensa"));
            Integer spd = Integer.parseInt(request.getParameter("velocidad"));
            Integer hp = Integer.parseInt(request.getParameter("vida"));
            ...
            if (miEjb.hasSixPokemon(miEjb.findTrainer(t.getName()))) {
                if (miEjb.insertPoke(p)) {
                    out.println("<h1>Nuevo Pokemon Añadido</h1>");
                } else {
                    out.println("<h1>Ese Pokemon ya existe</h1>");
                }
            }else{
                out.println("<h1>Ya tienes 6 Pokemon</h1>");
            }
            out.println("</body>");
            out.println("</html>");
        }
